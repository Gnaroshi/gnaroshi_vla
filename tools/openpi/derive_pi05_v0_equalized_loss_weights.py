#!/usr/bin/env python3
"""Derive the documented equal-contribution V0 weights from raw calibration."""
...
def main() -> None:
    ...
    if raw.get("V0_RAW_LOSS_CALIBRATION_COMPLETE") is not True:
        raise RuntimeError("raw V0 loss calibration is incomplete")
    ...
    weights = equalized_weights(raw["raw_metrics"])
    if args.shell:
        print(
            weights["state"], weights["chunk"], weights["executed"], weights["gripper"]
        )
    else:
