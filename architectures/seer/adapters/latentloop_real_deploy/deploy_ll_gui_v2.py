...
from architectures.seer.adapters.latentloop_real_deploy.gui_view import (
    DeploymentViewMixin,
)
from architectures.seer.adapters.latentloop_real_deploy.home_pose import (
    configure_home_pose,
)
...
class LatentLoopDeployGuiAppV2(DeploymentViewMixin, legacy_gui.DeployGuiApp):
    ...
    def _handle_keypress(self, event):
        ...
        if (panel is not None and str(event.widget).startswith(str(panel) + ".")
                and event.keysym.lower() != "x"):
            return
        return super()._handle_keypress(event)

    def _runtime_settings_label(self, control_freq, query_interval, rollout_policy):
        return (
            f"Applied target: {float(control_freq):g} Hz; "
    ...
    def write_session_files(self):
        ...
        manifest["home_configuration"] = self.cfg.home_configuration
    ...
    def _normalize_v2_results_metadata(self):
        ...
        metadata["home_configuration"] = self.cfg.home_configuration
...
def main():
    ...
    cfg = DeployConfig()
    home_configuration = configure_home_pose(
        cfg, gui_args.latentloop_artifact_manifest
    )
    print("[Seer home] " + json.dumps(home_configuration, sort_keys=True))
