...
def extract_task_information(file_name, path):
    ...
    match = re.search(pattern, file_name)
    if not match:
        print(f"[WARN] filename not matched: {file_name}")
        return None, None
    print(match.group(3).lower().replace("_", " "))
    return match.group(1).lower() if match else None, match.group(3).lower().replace("_", " ")
...
class DatasetConverter:
        ...
        for step_index in range(num_steps):
            ...
            with h5py.File(f'{step_dir}/other.h5', 'w') as h5_file:
                ...
                h5_file.create_dataset('language_instruction',
                                       data=np.array(language_instructions, dtype=h5py.string_dtype(encoding='utf-8')))
                # task emb
                # h5_file.create_dataset(name='task_emb', data=task_emb)
    ...
    def convert_origin_dataset_to_target(self, dataset_by_task):
        ...
        for episode_index in range(num_episodes):
            ...
            self.process_episode(episode_dir=episodes_dir,
                                 ...
                                 demo_data=dataset_by_task[task_index]['data'], episode_index=episode_index,
                                 episode_index_in_task=episode_index_in_task)
    ...
    #         task_name, task_language = extract_task_information(path_name, self.src_dir)
    #         demo_data = h5py.File(path_name, 'r')['data']
    #         num_episode = len(demo_data) #task 별 50개의 에피소드
    ...
    def run(self):
        ...
        paths = sorted(Path(self.src_dir).iterdir(), key=lambda p: p.name)
        for path in paths:
            path_name = str(path)
            task_name, task_language = extract_task_information(path_name, self.src_dir)
            ...
            keys = list(demo_ori.keys())
            keys.sort(key=lambda k: int(k.split('_')[-1]))  # demo_0..demo_49
            rng.shuffle(keys)  # <-- random.shuffle 대신 고정 RNG

            # selected_keys = keys[:5]  # 10%
            selected_keys = keys[:25]  # 10%
            demo_data = ShuffledSubsetGroup(f, demo_ori, selected_keys)
            num_episode = len(demo_data)
        ...
        write_json = (not dist.is_available()) or (not dist.is_initialized()) or dist.get_rank() == 0

        expected_episodes = sum(d['num_episode'] for d in dataset_by_task)

        episodes_root = self.tgt_dir / 'episodes'

        ep_dirs = []
        deadline = time.time() + 10.0
        while True:
            ep_dirs = sorted([p for p in episodes_root.iterdir() if p.is_dir()],
                             key=lambda p: int(p.name))
            if len(ep_dirs) >= expected_episodes or time.time() > deadline:
                break
            time.sleep(0.25)

        if len(ep_dirs) != expected_episodes:
            print(f"[WARN] episodes on disk={len(ep_dirs)} != expected={expected_episodes}")

        data_info = []
        for ep_dir in ep_dirs:
            meta_path = ep_dir / 'meta_info.h5'
            if meta_path.exists():
                with h5py.File(meta_path, 'r') as h5f:
                    steps = int(h5f['length'][()])
            else:
                steps_dir = ep_dir / 'steps'
                steps = len([s for s in steps_dir.iterdir() if s.is_dir()])
            data_info.append([ep_dir.name, steps])  # '000123', steps

        if write_json:
            json_name = self.tgt_dir.name  # 예: 'libero_90_converted'
            out_path = Path('./data_info') / f'{json_name}.json'
            with open(out_path, 'w') as f:
                json.dump(data_info, f)
            print(f'[OK] wrote {out_path} ({len(data_info)} episodes)')

            if len(data_info) != expected_episodes:
                print(f"[WARN] JSON episodes={len(data_info)} != expected={expected_episodes}")


class ShuffledSubsetGroup:
    def __init__(self, file_handle, group, selected_keys):
    ...
    def __getitem__(self, k):
        if isinstance(k, str) and k.startswith('demo_'):
            idx = int(k.split('_')[-1])
            true_key = self.selected_keys[idx]
            return self.group[true_key]
        elif isinstance(k, int):
            return self.group[self.selected_keys[k]]
        else:
            return self.group[k]

    def keys(self):
        return [f'demo_{i}' for i in range(len(self.selected_keys))]
...
def main(rank, port, num_worker, start_episode_idx=0, end_episode_idx=None):
    ...
    global dataset_name
    dataset_name = "libero_10"  # "libero_10" # << todo : libero_90 만 변환
    src_dir = f"/home/mingyujung/shared/robotics/LIBERO/datasets/{dataset_name}"  ## << todo : 디렉토리 변경
    tgt_dir = Path(f"/home/mingyujung/private/seer/seer_main/LIBERO_DATASETS/libero_10_converted_50/{dataset_name}_converted")
    tgt_dir.mkdir(exist_ok=True)
    ...
    dataset_converter = DatasetConverter(
        ...
        # None means the last episode. if not none, the dataset[end_episode_idx - 1] will be processed and the dataset[end_episode_idx] will not be processed
    )
    dataset_converter.run()
