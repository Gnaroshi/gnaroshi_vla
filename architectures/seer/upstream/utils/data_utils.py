...
class BaseDroidDataset(Dataset):
    ...
    def load_action(self, other_h5_file, max_rel_pos=0.02, max_rel_orn=0.05, magic_scaling_factor_pos=1.0,
...
class BaseLiberoDataset(Dataset):
    def __init__(
        ...
        if not dataset_info_path.is_file():
            raise FileNotFoundError(
                f"Missing LIBERO episode metadata: {dataset_info_path}. "
                "Set --libero_dataset_info_path to the converter-generated JSON file."
            )
    ...
    def load_action(self, other_file, max_rel_pos=0.02, max_rel_orn=0.05, magic_scaling_factor_pos=1.0,
                    magic_scaling_factor_orn=1.0):
        if self.load_libero_file == "h5":
...
def _latentloop_comparison_split(args, dataset):
    ...
    if not bool(getattr(args, "latentloop_comparison_protocol", 0)) or role == "full":
        return dataset, None
    ...
    manifest = {
        "protocol": "latentloop_q1_q2_comparison_v1",
        "role": role,
        "split_unit": "episode",
