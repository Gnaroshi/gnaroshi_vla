...
def get_parser(is_eval=False):
    ...
    parser.add_argument(
        ...
        help=(
            "Converted LIBERO dataset directory name used by libero_finetune. "
            "The default preserves the existing LIBERO-10 training path."
        ),
    )
    parser.add_argument(
        "--libero_dataset_info_path",
        type=str,
        default="",
        help=(
            "Optional absolute JSON episode-index path for the converted LIBERO "
            "dataset. Empty keeps the existing ./data_info/<dataset>.json lookup."
        ),
    )
    # pretrain
    ...
    parser.add_argument(
        "--lrnode_runtime_aligned_train",
        type=int,
        default=0,
        help=(
            "Opt-in shifted-context recurrent distillation over cache ages 1..H. "
            "This leaves the legacy one-step training path unchanged when disabled."
        ),
    )
    parser.add_argument("--lrnode_runtime_horizon", type=int, default=3)
    parser.add_argument("--lrnode_runtime_age3_weight", type=float, default=2.0)
    parser.add_argument(
        "--lrnode_frozen_teacher_eval_mode",
        type=int,
        default=0,
        help=(
            "When 1, keep the frozen Seer teacher/backbone in eval mode while "
            "training only the LR-NODE modules. Disabled by default for legacy parity."
        ),
    )
    parser.add_argument("--lrnode_gripper_distill_weight", type=float, default=0.0)
    parser.add_argument("--lrnode_overlap_weight", type=float, default=0.0)
    parser.add_argument("--lrnode_ensemble_weight", type=float, default=0.0)
    parser.add_argument("--lrnode_gripper_switch_weight", type=float, default=0.0)
    parser.add_argument("--lrnode_runtime_ensemble_temp", type=float, default=0.01)
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Enable inference-only FastV visual-token pruning inside Seer's causal transformer.",
    )
    parser.add_argument(
        "--fastv_prune_layer",
        "--fastv_k",
        dest="fastv_prune_layer",
        type=int,
        default=2,
        help="Zero-based transformer layer where pruning starts; layers [0, L) provide scores.",
    )
    parser.add_argument(
        "--fastv_prune_ratio",
        "--fastv_r",
        dest="fastv_prune_ratio",
        type=float,
        default=0.5,
        help="Fraction of Seer visual tokens removed from the pruning layer onward.",
    )
    parser.add_argument(
        "--fastv_score_mode",
        type=str,
        default="last_token_at_l",
        choices=[
            "text_mean_first_l",
            "last_token_at_l",
            "action_mean_first_l",
        ...
        help=(
            "Visual-token scoring contract: language queries averaged over the first L "
            "layers, the final sequence token at the last pre-pruning layer, or final-step "
            "action queries averaged over the first L layers. hf_last_action_at_l is a "
            "backward-compatible alias for last_token_at_l."
        ),
    )
    parser.add_argument(
        ...
        default=0,
        help=(
            "When 1, aggregate retained visual-token counts by timestep and camera. "
            "Keep disabled for paper latency runs because the aggregation itself adds "
            "GPU work inside the transformer forward."
        ),
    )
