...
def save_lrnode_run_snapshots(args, model, output_dir, repo_dir=None, extra_metadata=None):
    Path(output_dir).mkdir(parents=True, exist_ok=True)
    extra_metadata = dict(extra_metadata or {})

    def with_metadata(payload):
        return {**payload, **extra_metadata}

    write_json(
        os.path.join(output_dir, "args_snapshot.json"),
        with_metadata({k: v for k, v in vars(args).items()}),
    )
    write_json(
        os.path.join(output_dir, "lrnode_flags_snapshot.json"),
        with_metadata(collect_lrnode_flags(args)),
    )
    write_json(
        os.path.join(output_dir, "loss_weights_snapshot.json"),
        with_metadata(collect_loss_weights(args)),
    )
    write_json(
        os.path.join(output_dir, "model_trainable_params.json"),
        with_metadata(collect_model_trainable_params(model)),
    )
    write_json(
        os.path.join(output_dir, "freeze_status_snapshot.json"),
        with_metadata(collect_freeze_status(model)),
    )
    if repo_dir is None:
        repo_dir = os.getcwd()
    write_json(
        os.path.join(output_dir, "git_snapshot.json"),
        with_metadata(collect_git_snapshot(repo_dir)),
    )
