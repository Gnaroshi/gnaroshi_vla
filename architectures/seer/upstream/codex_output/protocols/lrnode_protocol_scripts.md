# LR-NODE protocol script 정리

작성일: 2026-06-16 KST

> 2026-06-20 문서 상태: script 목적/저장 위치 설명은 유효하다. 다만 이 문서의 예시는 `shifted_context`를 main처럼 적고 있으며, 최신 논의에서는 현재 `shifted_context` 구현을 slow 2-forward ablation으로 분리할 가능성이 높다. 최신 구현/실험 판단은 `codex_output/lrnode_current_state_handoff.md`와 `codex_output/README_LRNODE_DOCS.md`를 먼저 본다.
