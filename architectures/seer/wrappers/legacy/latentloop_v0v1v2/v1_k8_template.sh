...
OUTPUT="${CAMPAIGN_ROOT}/v1/eval/conditional_k8_200"
[[ -f "${CHECKPOINT}" ]] || fail "selected V1 checkpoint missing"
refuse_existing "${OUTPUT}"
"${S18_PYTHON}" -m torch.distributed.run --nnodes=1 --nproc_per_node=4 --master_port="${MASTER_PORT:-16280}" \
  ...
  --query-interval 8 --allow-conditional-interval \
