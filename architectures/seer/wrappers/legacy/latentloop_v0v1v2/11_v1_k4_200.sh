...
require_gate "${CAMPAIGN_ROOT}/v1/gates/v1_offline_gate.json" V1_OFFLINE_PASS
...
OUTPUT="${CAMPAIGN_ROOT}/v1/eval/fixed_k4_200"
...
"${S18_PYTHON}" -m torch.distributed.run --nnodes=1 --nproc_per_node=4 \
  --master_port="${MASTER_PORT:-16240}" "${REPO_ROOT}/tools/seer/run_latentloop_v1v2_eval_runtime.py" \
  ...
  --query-interval 4 --seed 42 --tasks 10 --episodes-per-task 20 --max-steps 600 \
