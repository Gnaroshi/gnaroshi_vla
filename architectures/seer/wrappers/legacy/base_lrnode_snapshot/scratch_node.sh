...
# Scratch LR-NODE teacher-student protocol.
# This is the main "from scratch + LR-NODE" setting:
...
#   - LR-NODE losses are detached from the teacher Seer/action head by default
...
# so they do not mix with older scratch_checkpoints_* experiments.

protocol_root="${LRNODE_PROTOCOL_ROOT:-/home/mingyujung/private/seer/seer_node3/runs_lrnode_protocol_20260616}"
save_checkpoint_path="${SAVE_CHECKPOINT_PATH:-${protocol_root}/train/scratch_node/}"
...
method_tag="${METHOD_TAG:-lrnode_scratch_ts_v1_lw05_aw01_g4}"
experiment_tag="${EXPERIMENT_TAG:-$(date +%Y%m%d_%H%M%S)}"
run_name="${RUN_NAME:-${which_server}_scratch_node_ts_${method_tag}_${experiment_tag}}"
export EXPERIMENT_TAG="${experiment_tag}"
export RUN_NAME="${run_name}"
latest_dir="${protocol_root}/train/_latest"

echo "[TRAIN INFO] script=scratch_node.sh"
...
echo "[TRAIN INFO] latest_pointer_after_success=${latest_dir}/scratch_node.env"

export CUDA_DEVICE_ORDER=PCI_BUS_ID
export CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-4,5,6,7}"
master_port="${MASTER_PORT:-10312}"
node=1
node_num="${NODE_NUM:-4}"
...
mkdir -p "${latest_dir}"
cat > "${latest_dir}/scratch_node.env" <<EOF
...
EOF
echo "[TRAIN INFO] wrote latest pointer: ${latest_dir}/scratch_node.env"
