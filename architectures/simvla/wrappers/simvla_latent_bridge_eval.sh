...
if [[ ${SIMVLA_LATENT_BRIDGE_EVAL_RUN:-0} != 1 ]]; then
  echo "Refusing LIBERO evaluation. Set SIMVLA_LATENT_BRIDGE_EVAL_RUN=1 after reviewing arguments." >&2
  exit 2
fi

exec "${PYTHON:-python}" -m architectures.simvla.adapters.latent_bridge.eval "$@"
