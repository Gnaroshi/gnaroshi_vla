...
def test_plan_adapter_k1_parity_uses_periodic_schedule() -> None:
    repo = Path(__file__).resolve().parents[1]
    source = (
        repo
        / "tests/fixtures/seer/retired_protocols/eval_latentloop_comparison.sh.txt"
    ).read_text()
    assert (
        'run_row action_loaded_k1 "${ACTION_CORRECTION_CKPT}" '
        'action_correction stepwise 1 0 0' in source
    )
    assert (
        'run_row nonrecurrent_loaded_k1 "${NONRECURRENT_CKPT}" '
        'anchor_bridge stepwise 1 0 0' in source
    )
    assert "source_lock_hotfix.json" in source
    assert "k1_schedule_and_optional_diagnostic_hotfix_v2" in source
    assert '"architectures/seer/upstream/utils/eval_utils_libero.py"' in source
    assert 'RESUME_EXISTING_ROWS="${RESUME_EXISTING_ROWS:-0}"' in source


def test_optional_gripper_logit_diagnostic_is_none_safe() -> None:
    repo = Path(__file__).resolve().parents[1]
    source = (
...
def test_sequential_resume_reuses_completed_parity_and_rows() -> None:
    repo = Path(__file__).resolve().parents[1]
    source = (
        repo
        / "tests/fixtures/seer/retired_protocols/run_latentloop_comparison_sequential.sh.txt"
    ).read_text(encoding="utf-8")
    assert "[RESUME] Reusing completed K1 parity smoke" in source
    assert 'RESUME_EXISTING_ROWS="${RESUME_EVAL_FROM_SELECTED}"' in source
    assert 'DATASET_ROOT="${LOCKED_DATASET_ROOTS[0]}"' in source
    assert "Ignoring ambient ROOT_DIR" in source
