...
OUTPUT="${CAMPAIGN_ROOT}/v1/train/e40"
refuse_existing "${OUTPUT}"
"${S18_PYTHON}" -m torch.distributed.run --nnodes=1 --nproc_per_node=4 \
  --master_port="${MASTER_PORT:-16140}" "${REPO_ROOT}/tools/seer/run_latentloop_v1_training_runtime.py" \
  --integration-status "${REPO_ROOT}/v1_runtime_integration_status.json" \
  --epochs 40 --output-root "${OUTPUT}" --teacher "${TEACHER}" --adapter-init "${ADAPTER}" \
