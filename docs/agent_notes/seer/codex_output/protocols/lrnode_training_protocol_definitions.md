# LR-NODE 학습 protocol 정의

이 문서는 `scripts/LIBERO_LONG/Seer/{scratch_node.sh,distill_node.sh,scratch_node_joint.sh}`의 의미를 코드/gradient 기준으로 고정하기 위한 정의서다.

> 2026-06-20 문서 상태: protocol/gradient coupling 정의는 유효하다. 단, target mode 해석은 최신 논의를 함께 봐야 한다. 현재 script default는 아직 `shifted_context`지만, 이 구현은 extra teacher full forward 때문에 느리다. dataset/dataloader 변경 없는 one-forward 대안은 `adjacent_sequence` 또는 향후 구현할 `last-pair adjacent`다. 최신 요약은 `codex_output/lrnode_current_state_handoff.md`와 `codex_output/README_LRNODE_DOCS.md`를 우선한다.
...
adapter/isolation 비교:

```text
동일 baseline ckpt full-forward
vs
